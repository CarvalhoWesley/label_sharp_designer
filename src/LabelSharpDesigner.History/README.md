...
Como todo o modelo de `LabelSharpDesigner.Core` é imutável (`record`, sempre `with { ... }` para
...
O `LabelSharpDesigner.UI.WinForms` (especificamente `LabelCanvasControl`) é o principal consumidor —
toda interação do usuário no editor (arrastar, redimensionar, apagar) passa por um `HistoryManager`.

## Dependências

Depende só de `LabelSharpDesigner.Core` (o `LabelDocument` que ele guarda em cada snapshot).
