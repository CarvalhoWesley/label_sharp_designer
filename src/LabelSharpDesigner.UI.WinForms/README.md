# LabelSharpDesigner.UI.WinForms
...
`LabelSharpDesigner.App` monta dentro das próprias janelas (e que qualquer outra aplicação WinForms
...
- **`LabelCanvasControl`** — o controle principal: a superfície onde o usuário edita a etiqueta
  ...
  automático (`AlignmentSnap`) e integra com o `LabelSharpDesigner.History` para undo/redo.
...
Ao manter os controles visuais num projeto separado de `LabelSharpDesigner.App`, é possível montar
...
Depende de `LabelSharpDesigner.Core`, `LabelSharpDesigner.Layout`, `LabelSharpDesigner.Expressions`,
`LabelSharpDesigner.History` e `LabelSharpDesigner.Rendering.Canvas`. Alvo
