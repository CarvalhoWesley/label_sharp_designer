# LabelSharpDesignerCore.App

## O que é

O **aplicativo final** do LabelSharpDesignerCore — o composition root que junta praticamente todos os
outros projetos do repositório em telas prontas: biblioteca de etiquetas, editor visual, exportação,
impressão e configurações. Se você só quer usar o LabelSharpDesignerCore "como está", é este `.exe` que
...
- **Modo biblioteca** (sem argumentos) — abre a `LibraryForm` sobre um `LibraryRepository`
  (`%APPDATA%\LabelSharpDesignerCore\Labels`): a tela de listar/criar/editar/duplicar/excluir/exportar/
...
- **Modo edição direta** (`--edit <caminho> [--readonly]`) — abre o `EditorForm` direto sobre um
  ...
  processo satélite via `LabelSharpDesignerCore.Legacy.Bridge` (`LaunchRequest.TryParse` é quem
  reconhece esses argumentos) — hoje um mecanismo opcional, relevante só para hosts web sem sessão
  de desktop interativa; a maioria dos hosts .NET Framework 4.x referencia este projeto direto (ver
...
- **`LibraryRepository`** — catálogo em disco de arquivos `.label`
  (`%APPDATA%\LabelSharpDesignerCore\Labels`): `Open()`/`OpenAt(dir)`, `List()`, `Create(pageConfig)`,
...
`EditorLayoutSettingsStore`) ficam em `%APPDATA%\LabelSharpDesignerCore\*.json` — são sobre *como você
...
`net48;net9.0-windows10.0.19041.0` (`WinExe`): **exige Windows**, mas é referenciável tanto por uma
aplicação .NET moderna quanto por uma .NET Framework 4.6.1+ (ver [INTEGRATION.md](../../INTEGRATION.md)).
