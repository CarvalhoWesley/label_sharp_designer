# LabelSharpDesigner.Rendering.ArgoxPpla
...
- **PPLA raster** (`PplaRasterBuilder`) — rasteriza a etiqueta **inteira** como uma imagem
  monocromática (reaproveitando o `LabelSharpDesigner.Rendering.Canvas`, o mesmo desenho usado no
...
Os bytes gerados aqui são enviados à impressora pelo `LabelSharpDesigner.PrintTransport.Windows`
(`WindowsRawPrintTransport`) — este projeto só gera os bytes, não sabe nada sobre spooler de
impressão do Windows.

## Dependências

Depende de `LabelSharpDesigner.Core`, `LabelSharpDesigner.Rendering.Abstractions`,
`LabelSharpDesigner.Rendering.Canvas` (para o modo raster) e `LabelSharpDesigner.Barcode`.
