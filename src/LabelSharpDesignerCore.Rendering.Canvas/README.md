# LabelSharpDesignerCore.Rendering.Canvas

## O que é

O **desenhador principal de verdade** do LabelSharpDesignerCore. Usa a biblioteca
...
- **`LabelSharpDesignerCore.Rendering.Png`** — usa este renderer para desenhar num `SKBitmap` e depois
  salvar como PNG.
- **`LabelSharpDesignerCore.Rendering.ArgoxPpla`** — usa este renderer para o modo "PPLA raster"
  (rasteriza a etiqueta inteira como imagem monocromática, para impressoras térmicas).
- **`LabelSharpDesignerCore.UI.WinForms`** — a aba "Pré-visualizar" do editor usa este renderer para
...
Depende de `LabelSharpDesignerCore.Core`, `LabelSharpDesignerCore.Rendering.Abstractions` (implementa
`IResolvedPayloadVisitor`) e `LabelSharpDesignerCore.Barcode` (para desenhar o raster de códigos de
