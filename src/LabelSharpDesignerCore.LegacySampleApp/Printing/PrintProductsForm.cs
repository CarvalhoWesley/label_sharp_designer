...
/// <summary>
/// Batch-print screen: pick products and how many labels each needs, pick which of this app's own
/// saved labels (<see cref="LabelRepository"/>, not the plugin's library) to print them on, and send
/// the result to a printer. Always mail-merge/batch mode (<see cref="LayoutEngine.ResolveBatch"/>).
///
...
/// (<see cref="PrintColumnsSettingsStore"/>) across label choices and app runs once first used — the
/// label's own column count is only ever a one-time suggestion for it.</para>
/// </summary>
public sealed class PrintProductsForm : Form
{
    ...
    /// <summary>User-configured product-field ↔ label-variable bindings, keyed by this app's own
    /// <see cref="LabelEntry.Id"/> then by variable name — see <see cref="VariableMappingForm"/>.</summary>
    private readonly Dictionary<string, Dictionary<string, ProductFieldSource>> _fieldMappings = VariableFieldMappingStore.Load();

    public PrintProductsForm(ProductRepository productRepository, LabelRepository labelRepository)
    {
        ...
        foreach (var printer in WindowsPrinterDiscovery.ListAvailable())
        {
        ...
        _formatCombo.Items.AddRange(new object[] { "PDF (via driver, qualquer impressora)", "PPLA nativo (Argox)", "PPLA raster (Argox — texto/imagem/QR)" });
    ...
    private void OnLabelSelectionChanged()
    {
        if (SelectedEntry() is { } entry)
        {
            ...
            _columnsUpDown.Value = Math.Max((int)_columnsUpDown.Minimum, Math.Min((int)_columnsUpDown.Maximum, suggested));
        }

        UpdatePreview();
    }

    private LabelEntry? SelectedEntry() => (_labelCombo.SelectedItem as LabelComboItem)?.Entry;
    ...
    private void RebuildForFormat()
    {
        ...
        if (SelectedFormat() != PrintFormat.Pdf)
        {
            var localTop = 0;
            ...
            _transferTypeCombo.Items.AddRange(new object[] { "Térmica direta", "Transferência térmica (ribbon)" });
    ...
    /// <summary>The selected label's document with "Colunas" swapped for whatever the user set here.</summary>
    ...
    private Dictionary<string, ProductFieldSource> ResolveMappingFor(LabelEntry entry)
    {
    ...
    /// matching common Portuguese/English aliases against its name.</summary>
    private static ProductFieldSource GuessDefaultSource(LabelVariable variable)
    {
        var key = variable.Name.Trim().ToLowerInvariant().Replace("_", string.Empty).Replace(" ", string.Empty);
        switch (key)
        {
            case "descricao":
            case "description":
            case "produto":
            case "nome":
                return ProductFieldSource.Description;
            case "preco":
            case "price":
            case "valor":
                return ProductFieldSource.Price;
            case "codigobarras":
            case "barcode":
            case "ean":
            case "codigo":
                return ProductFieldSource.Barcode;
            default:
                return ProductFieldSource.LabelDefault;
        }
    }
    ...
    private IReadOnlyList<IReadOnlyDictionary<string, object?>> BuildRecords(LabelEntry entry)
    {
    ...
    private IReadOnlyList<ResolvedDocument> ResolveBatchRows()
    {
        ...
        {
            return Array.Empty<ResolvedDocument>();
        }

        var records = BuildRecords(entry);
        return records.Count == 0 ? Array.Empty<ResolvedDocument>() : _layoutEngine.ResolveBatch(document, records);
    }
    ...
    /// <summary>Sends every physical row to the printer, always with a single copy per row since the
    /// requested quantity is already baked into how many records were tiled into these rows.</summary>
    private int PrintRows(PrintFormat format, string? target, IReadOnlyList<ResolvedDocument> rows)
    {
        switch (format)
        {
    ...
    private sealed record LabelComboItem(LabelEntry Entry)
    {
