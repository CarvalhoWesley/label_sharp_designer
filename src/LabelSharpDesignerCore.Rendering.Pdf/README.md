# LabelSharpDesignerCore.Rendering.Pdf
...
- **`BarcodeDrawing`** — embute o raster do código de barras (gerado por `LabelSharpDesignerCore.Barcode`)
...
`IResolvedPayloadVisitor`) e `LabelSharpDesignerCore.Barcode`. Multi-targeta `netstandard2.0;net9.0`,
sem dependência de Windows.

## Quem usa este projeto

`LabelSharpDesignerCore.PrintTransport.Windows` (`WindowsPdfPrintTransport`) manda o PDF gerado aqui
para o driver de impressão do Windows; `LabelSharpDesignerCore.App` (`ExportDialogForm`) também usa este
