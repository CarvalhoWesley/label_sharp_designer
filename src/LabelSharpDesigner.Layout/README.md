...
- **`LayoutEngine.Resolve`** — resolve **um** documento: converte milímetros para pixels (dots) de
  acordo com o DPI da página, calcula `{{ expressões }}` (via `LabelSharpDesigner.Expressions`),
...
Depende de `LabelSharpDesigner.Core` (o modelo que ele resolve) e
`LabelSharpDesigner.Expressions` (para calcular as `{{ }}`). Multi-targeta
